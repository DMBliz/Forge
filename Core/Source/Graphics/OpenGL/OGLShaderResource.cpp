...
namespace Forge
{
	...
	void OGLShaderResource::LoadShader(const String& filename)
	{
		shaderSource = FileSystem::ReadFile(filename).GetString();
	...
	String OGLShaderResource::GetBlock(const String& blockName)
	{
	...
	void OGLShaderResource::Compile()
	{
		unsigned vs;
	...
	void OGLShaderResource::Use()
	{
		glCheck(glUseProgram(programID));
	}

	void OGLShaderResource::ResolveUniformLocations(ShaderUniforms& uniforms)
	{
	...
	void OGLShaderResource::SetValuesToUniforms(const ShaderUniforms& uniforms)
	{
		for(int i = 0; i < uniforms.GetSize(); i++)
		{
	...
	void OGLShaderResource::SetValueToUniform(const UniformDescription& desc)
	{
		switch (desc.type)
		{
	...
	void OGLShaderResource::SetSystemValueToUniform(UniformDescription& desc)
	{
